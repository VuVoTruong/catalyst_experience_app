...
import (
	...
	"catalyst/responses"
	s "catalyst/server"
	authTokenservice "catalyst/services/auth_token"
	"fmt"
	"net/http"
...
func (authHandler *AuthHandler) Login(c echo.Context) error {
	...
	authTokenService := authTokenservice.NewAuthTokenService(authHandler.server.Config)
	accessToken, exp, err := authTokenService.CreateAccessToken(&user)
	if err != nil {
		return err
	}
	refreshToken, err := authTokenService.CreateRefreshToken(&user)
	if err != nil {
		return err
	}
